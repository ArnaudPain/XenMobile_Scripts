...
Function ProcessLUsersG
{
	If($MSWORD -or $PDF)
	{
		...
		WriteWordLine 1 0 "Users Groups Server"
	}
	ElseIf($Text)
	{
		Write-Verbose "$(Get-Date): `t`t`tProcessing Users Groups information"
...
Function OutputLUsersG
{
	...
	for ($v=0;$v -lt $count; $v++)
	{
		ForEach($userGroups in $user) 
		{
			...
			{
				## Add the required key/values to the hashtable
				$WordTableRowHash = @{ name = $user.userGroups[$CurrentLUGIndex].name;
									   uniqueName = $user.userGroups[$CurrentLUGIndex].uniqueName;
									   uniqueId = $user.userGroups[$CurrentLUGIndex].uniqueId;
									   domainname = $user.userGroups[$CurrentLUGIndex].domainname;
									 }
				## Add the hash to the array
			...
			ElseIf($Text)
			{
				Line 0 "Name`t`t`t: " $user.userGroups[$CurrentLUGIndex].name
				Line 0 "Unique Name`t`t: " $user.userGroups[$CurrentLUGIndex].uniqueName
				Line 0 "Unique Id`t`t: " $user.userGroups[$CurrentLUGIndex].uniqueId
				Line 0 "Domain Name`t`t: " $user.userGroups[$CurrentLUGIndex].domainname
				Line 0 ""
				$CurrentLUGIndex++
			}
			ElseIf($HTML)
			{
				$HighlightedCells = $htmlwhite
				$rowdata += @(,(
				$user.userGroups[$CurrentLUGIndex].name,$htmlwhite,
				$user.userGroups[$CurrentLUGIndex].uniqueName,$htmlwhite,
				$user.userGroups[$CurrentLUGIndex].uniqueId,$htmlwhite,
				$user.userGroups[$CurrentLUGIndex].domainname,$htmlwhite))
				$CurrentLUGIndex++
			}	
		}
	}
