# Script to Add Applications to a Delivery Group
...
<#
.SYNOPSIS
Add Applications to Delivery Group in XenMobile

.DESCRIPTION
This script will log into a XenMobile Server, list the Delivery Groups, ask for: DG, nb of Applications to add and name of each
...
$host.ui.RawUI.ForegroundColor = "Yellow"
Write-host "$(Get-Date): Extract Delivery Groups Applications"
$host.ui.RawUI.ForegroundColor = "white"
...
Write-host "Applications already configured for"$dgname":"
$host.ui.RawUI.ForegroundColor = "Green"
...
{
write-output $dglistdata.dglistdata.dglist[$new].applications.name
...
$host.ui.RawUI.ForegroundColor = "white"
$count = Read-Host -Prompt "How many Applications do you want to add"

# Add each new Action to currentact.txt
for($v=0;$v -lt $count;$v++)
{
$w=$v+1
$appname = Read-Host -Prompt "Please provide Applications $w name (respect the case)"
$appname >> currentapp.txt
$apprequired = Read-Host -Prompt "Is Application $appname Required (true/false)?"
$apprequired >> currentappreq.txt
}
...
$addapp=Invoke-RestMethod -Uri "https://${XMS}:4443/xenmobile/api/v1/deliverygroups" -Body $devpol -Headers $headers -Method Put
if($test -match "True")
{
del addgrp.txt 
del groupe.txt 
...
del dglist.txt
$host.ui.RawUI.ForegroundColor = "Green"
write-host "$(Get-Date):	Delivery Groups Updated"
$host.ui.RawUI.ForegroundColor = "White"
#endregion