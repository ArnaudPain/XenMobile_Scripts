...
.PARAMETER CoverPage
	...
	Valid input is:
		...
		banded (Word 2010/2013/2016. Doesn't work in 2013 or 2016, works in 2010)
...
.EXAMPLE
	...
	banded for the Cover Page format.
...
.EXAMPLE
	PS C:\PSScript > .\XMS-Report.ps1 -SmtpServer smtp.office365.com -SmtpPort 587 -UseSSL -From Webster@CarlWebster.com -To ITGroup@CarlWebster.com
...
.NOTES
	NAME: XMS-Report.ps1
	VERSION: 1.00
	AUTHOR: Arnaud Pain, Michael B. Smith, Iain Brighton, Jeff Wouters, Barry Schiffer, Jim Moyle
	...
	Switch ($CultureCode)
	{
		...
		'en-'	{
				If($xWordVersion -eq $wdWord2013 -or $xWordVersion -eq $wdWord2016)
				{
					...
					"Semaphore", "banded", "Slice (Dark)", "Slice (Light)", "ViewMaster",
					"Whisp")
				}
				ElseIf($xWordVersion -eq $wdWord2010)
				{
					...
					"Perspective", "Pinstripes", "Puzzle", "banded", "Stacks", "Tiles", "Transcend")
				}
			}
...
Function SetupWord
{
	...
	If($Script:WordCultureCode -ne "en-")
	{
		...
		If($CPChanged)
		{
			Write-Verbose "$(Get-Date): Changed Default Cover Page from banded to $($CoverPage)"
		}
	}
...
Function OutputSampleText
{
	If($MSWORD -or $PDF)
	{
		...
		WriteWordLine 0 1 "Local Users Groups" "" $null 0 $true $false
		...
		WriteWordLine 0 0 ""
		WriteWordLine 0 0 ""
		WriteWordLine 0 0 "The current version of the script is 1.0,  updates are planned to be implemented in the next few weeks."
		WriteWordLine 0 0 ""
		WriteWordLine 0 0 "If you have any questions feel free to send me email: arnaud.pain@arnaud.biz"
		WriteWordLine 0 0 ""
...
Write-Verbose "$(Get-Date): Chapter $Chapter/$Chapters Certificates" 
	...
	Try
	{
		...
		## If we need to retrieve the Key might as well just use WMI.
...
$count = $cert.certificate.length
	...
	ForEach($certificate in $cert) 
	{
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($certificate.certificate[$CurrentCertificateIndex].name,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].description,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].remainingdays,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].validfrom,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].validto,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].type,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].isactive,$htmlwhite,$certificate.certificate[$CurrentCertificateIndex].privatekey,$htmlwhite))
			$CurrentCertificateIndex++
			}	
		}
	$new++
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Display Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Remaining Days',($htmlsilver -bor $htmlbold),'Valid From',($htmlsilver -bor $htmlbold),,'Valid To',($htmlsilver -bor $htmlbold),,'Type',($htmlsilver -bor $htmlbold),'Is Active',($htmlsilver -bor $htmlbold),'Private Key',($htmlsilver -bor $htmlbold))
...
$count = $lic.cpLicenseServer.LicenseList.length
	...
	ForEach($cpLicenseServer in $lic) 
	{
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($cpLicenseServer.cpLicenseServer.serveraddress,$htmlwhite,$cpLicenseServer.cpLicenseServer.servertype,$htmlwhite,$cpLicenseServer.cpLicenseServer.licenseType,$htmlwhite,$cpLicenseServer.cpLicenseServer.licenselist.remainingdays,$htmlwhite,$cpLicenseServer.cpLicenseServer.licenselist.licensesinuse,$htmlwhite,$cpLicenseServer.cpLicenseServer.licenselist.licensesavailable,$htmlwhite))
		}	
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Server Address',($htmlsilver -bor $htmlbold),'Server Type',($htmlsilver -bor $htmlbold),'License Type',($htmlsilver -bor $htmlbold),'Remaining Days',($htmlsilver -bor $htmlbold),'Licenses In Use',($htmlsilver -bor $htmlbold),'Licenses Available',($htmlsilver -bor $htmlbold))
...
$count = ConvertFrom-Json -inputobject $app.applicationlistdata.totalmatchcount
	...
	ForEach($applicationlistdata in $app) 
	{
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].name,$htmlwhite,$applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].description,$htmlwhite,$applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].createdon,$htmlwhite,$applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].lastupdated,$htmlwhite,$applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].disabled,$htmlwhite,$htmlwhite,$applicationlistdata.applicationlistdata.applist[$CurrentApplicationsIndex].apptype,$htmlwhite))
			$CurrentApplicationsIndex++;
		}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Created On',($htmlsilver -bor $htmlbold),'Last Update',($htmlsilver -bor $htmlbold),'Disabled',($htmlsilver -bor $htmlbold),'App Type',($htmlsilver -bor $htmlbold))
...
$count = $netscaler.aglist.length
		...
		for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($aglist.aglist[$v].name,$htmlwhite,$aglist.aglist[$v].alias,$htmlwhite,$aglist.aglist[$v].url,$htmlwhite,$aglist.aglist[$v].passwordrequired,$htmlwhite,$aglist.aglist[$v].logontype,$htmlwhite,$aglist.aglist[$v].default,$htmlwhite))
			$CurrentNetScalerIndex++;
			}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Name',($htmlsilver -bor $htmlbold),'Alias',($htmlsilver -bor $htmlbold),'URL',($htmlsilver -bor $htmlbold),'Password Required',($htmlsilver -bor $htmlbold),'Logon Type',($htmlsilver -bor $htmlbold),'Default',($htmlsilver -bor $htmlbold))
...
$count=$ldap.adlist.length
		...
		for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($adlist.adlist[$v].domain,$htmlwhite,$adlist.adlist[$v].userbasedn,$htmlwhite,$adlist.adlist[$v].groupbasedn,$htmlwhite,$adlist.adlist[$v].port,$htmlwhite,$adlist.adlist[$v].username,$htmlwhite,$adlist.adlist[$v].primaryhost,$htmlwhite,$adlist.adlist[$v].secondaryhost,$htmlwhite,$adlist.adlist[$v].usesecure,$htmlwhite,$adlist.adlist[$v].globalcatalogport,$htmlwhite,$adlist.adlist[$v].usersearchby,$htmlwhite,$adlist.adlist[$v].domainalias,$htmlwhite))
			$CurrentLDAPIndex++;
		}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Domain',($htmlsilver -bor $htmlbold),'User Base DN',($htmlsilver -bor $htmlbold),'Group Base DN',($htmlsilver -bor $htmlbold),'Port',($htmlsilver -bor $htmlbold),'Username',($htmlsilver -bor $htmlbold),'Primary Host',($htmlsilver -bor $htmlbold),'Secondary Host',($htmlsilver -bor $htmlbold),'Use Secure',($htmlsilver -bor $htmlbold),'Global Catalog Port',($htmlsilver -bor $htmlbold),'User Search By',($htmlsilver -bor $htmlbold),'Domain Alias',($htmlsilver -bor $htmlbold))
...
$count = $dev.matchedRecords
	...
	ForEach($devices in $dev.filteredDevicesDataList) 
	{
		...
		ElseIf($Text)
			{
			Line 0 "Device ID`t`t`t: " $dev.filteredDevicesDataList[$CurrentDevicesIndex].id
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($dev.filteredDevicesDataList[$CurrentDevicesIndex].id,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].platform,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].devicemodel,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].managed,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].mamregistered,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].mdmknown,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].devicename,$htmlwhite,$dev.filteredDevicesDataList[$CurrentDevicesIndex].username,$htmlwhite))
			$CurrentDevicesIndex++
			}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Device ID',($htmlsilver -bor $htmlbold),'Device Platform',($htmlsilver -bor $htmlbold),'Device Model',($htmlsilver -bor $htmlbold),'Device Managed',($htmlsilver -bor $htmlbold),'Device MAM',($htmlsilver -bor $htmlbold),'Device MDM',($htmlsilver -bor $htmlbold),'Device Name',($htmlsilver -bor $htmlbold),'Enrolled User',($htmlsilver -bor $htmlbold))
...
$count = $notif.list.length
	...
	ForEach($list in $notif) 
	{
		...
		ElseIf($HTML)
			...
			#$CurrentNotSrvIndex=0
			$rowdata += @(,($list.list.name[$CurrentNotSrvIndex],$htmlwhite,$list.list.id[$CurrentNotSrvIndex],$htmlwhite,$list.list.active[$CurrentNotSrvIndex],$htmlwhite,$list.list.server[$CurrentNotSrvIndex],$htmlwhite,$list.list.servertype[$CurrentNotSrvIndex],$htmlwhite))
			$CurrentNotSrvIndex++
			}	
	#$CurrentNotSrvIndex++
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Name',($htmlsilver -bor $htmlbold),'ID',($htmlsilver -bor $htmlbold),'Active',($htmlsilver -bor $htmlbold),'Server',($htmlsilver -bor $htmlbold),'Server Type',($htmlsilver -bor $htmlbold))
...
#List Local User Groups configuration
$user=Invoke-RestMethod -Uri https://${XMS}:4443/xenmobile/api/v1/localusersgroups -Headers $headers -Method Get -Verbose:$false
...
$Chapter++
Write-Verbose "$(Get-Date): Chapter $Chapter/$Chapters Local Users Groups"
	...
	If(!$? -or $Null -eq $Script:LUG)
	{
		Write-Warning "No Local Users Groups were retrieved."
		If($MSWORD -or $PDF)
		{
			WriteWordLine 0 0 "Warning: No Local Users Groups were retrieved" "" $Null 0 $False $True
		}
		ElseIf($Text)
		{
			Line 0 "Warning: No Local Users Groups were retrieved"
		}
		ElseIf($HTML)
		{
			WriteHTMLLine 0 0 "Warning: No Local Users Groups were retrieved" "" $Null 0 $htmlbold
		}
		Return $False
	}
	ElseIf($? -and $Null -eq $Script:LUG)
	{
		Write-Warning "Local Users Groups retrieval was successful but no Local Users Groups were returned."
		If($MSWORD -or $PDF)
		{
			WriteWordLine 0 0 "Local Users Groups retrieval was successful but no Local Users Groups were returned." "" $Null 0 $False $True
		}
		ElseIf($Text)
		{
			Line 0 "Local Users Groups retrieval was successful but no Local Users Groups were returned."
		}
		ElseIf($HTML)
		{
			WriteHTMLLine 0 0 "Local Users Groups retrieval was successful but no Local Users Groups were returned." "" $Null 0 $htmlbold
		}
		Return $False
	}
	Else
	{
		...
		Write-Verbose "$(Get-Date): `t`t$($count) Local Users Groups configuration found"
		Return $True
	}
}

Function ProcessLUsersG
{
	If($MSWORD -or $PDF)
	{
		$Script:Selection.InsertNewPage()
		Write-Verbose "$(Get-Date): `t`t`tProcessing Local Users Groups information"
		WriteWordLine 1 0 "Local Users Groups Server"
	}
	ElseIf($Text)
	{
		Write-Verbose "$(Get-Date): `t`t`tProcessing Local Users Groups information"
		Line 0 ""
		Line 0 "Local Users Groups"
	}
	ElseIf($HTML)
	{
		Write-Verbose "$(Get-Date): `t`t`tProcessing Local Users Groups information"
		WriteHTMLLine 0 0 ""
		WriteHTMLLine 1 0 "Local Users Groups"
		WriteHTMLLine 0 0 ""
	}
	
	OutputLUsersG $Script:LUG $Script:NumLUG
	If($MSWORD -or $PDF)
	{
		OutputLUsersGNoInternalGridLines $Script:LUG $Script:NumLUG
	}
}

Function OutputLUsersG
{
#Bypass certificate verification to enable access with XMS IP Address
[System.Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}
...
$count = $user.users.length
	...
	{
		WriteWordLine 0 1 ""
		WriteWordLine 0 1 "$count Local Users Groups found on the XMS Server $XMS"
		WriteWordLine 0 1 ""
	...
	{
		Line 0 ""
		Line 0 "`t$count Local Users Groups configuration found on the XMS Server $XMS"
		Line 0 ""
	}
	...
	ForEach($users in $user) 
	{
		If($MSWord -or $PDF)
			{
			## Add the required key/values to the hashtable
			$WordTableRowHash = @{ username = $user.users[$CurrentLUGIndex].username;
								   groups = $user.users[$CurrentLUGIndex].groups;
								   role = $user.users[$CurrentLUGIndex].role;
								   aduser = $user.users[$CurrentLUGIndex].aduser;
								   createdon = $user.users[$CurrentLUGIndex].createdon;
								   lastAuthenticated = $user.users[$CurrentLUGIndex].lastAuthenticated;
								   domainname = $user.users[$CurrentLUGIndex].domainname;
								 }
			## Add the hash to the array
			$LUGWordTable += $WordTableRowHash;
			$CurrentLUGIndex++
			}
		
		ElseIf($Text)
			{
			Line 0 "Username`t`t: " $user.users[$CurrentLUGIndex].username
			Line 0 "Group`t`t`t: " $user.users[$CurrentLUGIndex].groups
			Line 0 "Role`t`t`t: " $user.users[$CurrentLUGIndex].role
			Line 0 "AD User`t`t`t: " $user.users[$CurrentLUGIndex].aduser
			Line 0 "Created On`t`t: " $user.users[$CurrentLUGIndex].createdon
			Line 0 "Last Authentication`t: " $user.users[$CurrentLUGIndex].lastAuthenticated
			Line 0 "Domain Name`t`t: " $user.users[$CurrentLUGIndex].domainname
			Line 0 ""
			$CurrentLUGIndex++
			}
		...
		ElseIf($HTML)
			...
			$rowdata += @(,($user.users[$CurrentLUGIndex].username,$htmlwhite,$user.users[$CurrentLUGIndex].groups[$CurrentLUGIndex-1],$htmlwhite,$user.users[$CurrentLUGIndex].role,$htmlwhite,$user.users[$CurrentLUGIndex].aduser,$htmlwhite,$user.users[$CurrentLUGIndex].createdon,$htmlwhite,$user.users[$CurrentLUGIndex].lastAuthenticated,$htmlwhite,$user.users[$CurrentLUGIndex].domainname,$htmlwhite))
			$CurrentLUGIndex++
			}	
	}
	}
	...
	{
		## Add the table to the document, using the hashtable (-Alt is short for -AlternateBackgroundColor!)
		$Table = AddWordTable -Hashtable $LUGWordTable `
		-Columns username, groups, role, aduser, createdon, lastAuthenticated, domainname `
		-Headers "Username", "Group", "Role", "AD User", "Created On", "Last Authentication", "Domain Name" `
		-Format -155 `
		...
		$Table.Columns.Item(4).Width = 100;
		$Table.Columns.Item(5).Width = 100;
		$Table.Columns.Item(6).Width = 100;
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Username',($htmlsilver -bor $htmlbold),'Group',($htmlsilver -bor $htmlbold),'Role',($htmlsilver -bor $htmlbold),'AD User',($htmlsilver -bor $htmlbold),'Created On',($htmlsilver -bor $htmlbold),'Last Authentication',($htmlsilver -bor $htmlbold),'Domain Name',($htmlsilver -bor $htmlbold))
		$msg = "$count Local Users Groups configuration found"
		FormatHTMLTable $msg "auto" -rowArray $rowdata -columnArray $columnHeaders
		WriteHTMLLine 0 0 ""
	}
}
...
Function Table
	...
	for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML) 
			...
			$rowdata += @(,($dglistdata.dglistdata.dglist[$CurrentDgroupIndex].id,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].name,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].description,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].disabled,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbsuccess,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbfailure,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbpending,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].enrollmentprofilename,$htmlwhite))
			$CurrentDgroupIndex++
			}	
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('ID',($htmlsilver -bor $htmlbold),'Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Disabled',($htmlsilver -bor $htmlbold),'Nb of Success',($htmlsilver -bor $htmlbold),'Nb of Failure',($htmlsilver -bor $htmlbold),'Nb of Pending',($htmlsilver -bor $htmlbold),'Enrollment Profile Name',($htmlsilver -bor $htmlbold))
...
$count = $dgroup.dglistdata.dglist.length
	...
	for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML) 
			...
			$rowdata += @(,($dglistdata.dglistdata.dglist[$CurrentDgroupIndex].id,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].name,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].description,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].disabled,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].applications.name,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].applications.required,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].devicepolicies.name,$htmlwhite,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbsuccess,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbfailure,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].nbpending,$htmlwhite,$dglistdata.dglistdata.dglist[$CurrentDgroupIndex].enrollmentprofilename,$htmlwhite))
			$CurrentDgroupIndex++
			}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('ID',($htmlsilver -bor $htmlbold),'Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Disabled',($htmlsilver -bor $htmlbold),'Application Name',($htmlsilver -bor $htmlbold),'Required',($htmlsilver -bor $htmlbold),'Device Policies',($htmlsilver -bor $htmlbold),'Nb of Success',($htmlsilver -bor $htmlbold),'Nb of Failure',($htmlsilver -bor $htmlbold),'Nb of Pending',($htmlsilver -bor $htmlbold),'Enrollment Profile Name',($htmlsilver -bor $htmlbold))
		$msg = "$count Delivery Groups configuration found"
		FormatHTMLTable $msg "auto" -rowArray $rowdata -columnArray $columnHeaders
		WriteHTMLLine 0 0 ""
	}
...
$count = $enrollment.enrollmentmodes.enrollmentmodes.length
	...
	for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML) 
			...
			$rowdata += @(,($enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].name,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].modedisplayname,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].validdurationmillis,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].maxtry,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].enabled,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].shpmode,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].defaultable,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].requiringidentification,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].requiringauthentication,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].requiringtoken,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].requiringsecret,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].requiringcertificate,$htmlwhite,$enrollmentmodes.enrollmentmodes.enrollmentmodes[$CurrentemodeIndex].default,$htmlwhite))
			$CurrentemodeIndex++
			}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Name',($htmlsilver -bor $htmlbold),'Display Name',($htmlsilver -bor $htmlbold),'Validation Duration (ms)',($htmlsilver -bor $htmlbold),'Max Try',($htmlsilver -bor $htmlbold),'Enabled',($htmlsilver -bor $htmlbold),'Self Help Portal',($htmlsilver -bor $htmlbold),'Default Table',($htmlsilver -bor $htmlbold),'Require Identification',($htmlsilver -bor $htmlbold),'Require Authentication',($htmlsilver -bor $htmlbold),'Require Token',($htmlsilver -bor $htmlbold),'Require Secret',($htmlsilver -bor $htmlbold),'Require Certificate',($htmlsilver -bor $htmlbold),'Default',($htmlsilver -bor $htmlbold))
...
$count = $clientprop.allclientproperties.length
	...
	for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML) 
			...
			$rowdata += @(,($allclientproperties.allclientproperties[$CurrentcpropIndex].displayname,$htmlwhite,$allclientproperties.allclientproperties[$CurrentcpropIndex].description,$htmlwhite,$allclientproperties.allclientproperties[$CurrentcpropIndex].key,$htmlwhite,$allclientproperties.allclientproperties[$CurrentcpropIndex].value,$htmlwhite,$allclientproperties.allclientproperties[$CurrentcpropIndex].predefined,$htmlwhite))
			$CurrentcpropIndex++
			}	
	}
	}
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('Display Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Key',($htmlsilver -bor $htmlbold),'Value',($htmlsilver -bor $htmlbold),'Predefined',($htmlsilver -bor $htmlbold))
...
$count = $srvprop.allewproperties.length
	...
	for ($v=0;$v -lt $count; $v++)
	{
		...
		ElseIf($HTML) 
			...
			$rowdata += @(,($allewproperties.allewproperties[$CurrentspropIndex].id,$htmlwhite,$allewproperties.allewproperties[$CurrentspropIndex].name,$htmlwhite,$allewproperties.allewproperties[$CurrentspropIndex].value,$htmlwhite,$allewproperties.allewproperties[$CurrentspropIndex].displayname,$htmlwhite,$allewproperties.allewproperties[$CurrentspropIndex].description,$htmlwhite,$allewproperties.allewproperties[$CurrentspropIndex].defaultvalue,$htmlwhite))
			$CurrentspropIndex++
			}	
	}
	}	
	...
	ElseIf($HTML)
	{
		$columnHeaders = @('ID',($htmlsilver -bor $htmlbold),'Name',($htmlsilver -bor $htmlbold),'Value',($htmlsilver -bor $htmlbold),'Display Name',($htmlsilver -bor $htmlbold),'Description',($htmlsilver -bor $htmlbold),'Default Value',($htmlsilver -bor $htmlbold))
...
Else
{
	Write-Verbose "$(Get-Date): Local Users Groups cannot be done."
}
$chapter++
