# Script to Add Device Policies to a Delivery Group
...
<#
.SYNOPSIS
Add Device Policies to Delivery Group in XenMobile

.DESCRIPTION
This script will log into a XenMobile Server, list the Delivery Groups, ask for: DG, nm of Device Policies to add and name of each
...
#region ask for Delivery Groups
$host.ui.RawUI.ForegroundColor = "white"
$dgname = Read-Host -Prompt "Please provide the Name of the Delivery Group"

Write-host "Device Policies already configured for"$dgname":"
$host.ui.RawUI.ForegroundColor = "Green"
...
{
write-output $dglistdata.dglistdata.dglist[$new].devicepolicies.name
$dglistdata.dglistdata.dglist[$new].devicepolicies.name > currentdev.txt
}
$new++
}
}
...
$host.ui.RawUI.ForegroundColor = "white"
$count = Read-Host -Prompt "How many Devices Policies do you want to add"

# Add each new Device Policy to currentdev.txt
for($v=0;$v -lt $count;$v++)
{
$w=$v+1
$name = Read-Host -Prompt "Please provide Policy $w name"
$name >> currentdev.txt
}

# Create result.txt file with formated query
$acount=0
foreach ($value in get-content currentdev.txt)
{
$z=
'{
"name": "'+ $value +'"
...
# Modify file to delete last character ',' on last line
$pol = get-content cdevresult.txt
$pol[0] = $pol[0] -replace ','
$pol[$pol.length - 1 ] = $pol[$pol.length - 1 ] -replace ','
$pol | set-content addpol.txt
#endregion
...
#del dglist.txt
$host.ui.RawUI.ForegroundColor = "Green"
write-host "$(Get-Date):	Device Policies added to Delivery Groups"
$host.ui.RawUI.ForegroundColor = "White"
#endregion		